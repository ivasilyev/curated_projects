...
export LOGS_DIR="${QIIME2_DIR}/logs"
mkdir -p "${LOGS_DIR}"
...
qiime demux summarize --verbose \
  ...
  |& tee "${LOGS_DIR}/demux summarize demux_PE_reads.log"
...
qiime dada2 denoise-paired --verbose \
  ...
  |& tee "${LOGS_DIR}/dada2 denoise-paired.log"
...
qiime feature-classifier classify-sklearn --verbose \
  ...
  |& tee "${LOGS_DIR}/feature-classifier classify-sklearn.log"
...
qiime metadata tabulate --verbose \
  ...
  |& tee "${LOGS_DIR}/metadata tabulate classified_taxonomy.log"
...
qiime alignment mafft --verbose \
  ...
  |& tee "${LOGS_DIR}/alignment mafft.log"
...
qiime alignment mask --verbose \
  ...
  |& tee "${LOGS_DIR}/alignment mask.log"
...
qiime phylogeny fasttree --verbose \
  ...
  |& tee "${LOGS_DIR}/phylogeny fasttree.log"
...
qiime phylogeny midpoint-root --verbose \
  ...
  |& tee "${LOGS_DIR}/phylogeny midpoint-root.log"
...
qiime diversity core-metrics-phylogenetic --verbose --p-sampling-depth 1000 \
  ...
  |& tee "${LOGS_DIR}/diversity core-metrics-phylogenetic.log"
...
qiime diversity alpha-rarefaction --verbose --p-max-depth 2000 \
  ...
  |& tee "${LOGS_DIR}/diversity alpha-rarefaction.log"
...
qiime vsearch join-pairs --verbose --p-allowmergestagger \
  ...
  |& tee "${LOGS_DIR}/vsearch join-pairs.log"
...
qiime quality-filter q-score --verbose \
  ...
  |& tee "${LOGS_DIR}/quality-filter q-score.log"
...
qiime vsearch dereplicate-sequences --verbose \
  ...
  |& tee "${LOGS_DIR}/vsearch dereplicate-sequences.log"
...
qiime vsearch cluster-features-closed-reference --verbose --p-perc-identity 0.97 \
  ...
  |& tee "${LOGS_DIR}/vsearch cluster-features-closed-reference.log"
...
qiime tools export \
  ...
  --output-path "biom" \
  |& tee "${LOGS_DIR}/tools export base biom.log"
...
biom convert --to-tsv \
  ...
  |& tee "${LOGS_DIR}/biom convert base tsv.log"
# With taxa
biom convert --to-tsv \
  --input-fp "biom/OTUs_with_taxa.biom" \
  --output-fp "biom/OTUs_with_taxa.tsv" \
  --header-key taxonomy \
  |& tee "${LOGS_DIR}/biom convert taxa tsv.log"
...
qiime tools export \
  ...
  |& tee "${LOGS_DIR}/tools export fasta.log"
