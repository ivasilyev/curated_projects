...
bowtie2 \
    ...
    --threads "$(nproc)"  `# Вести расчеты на всех доступных потоках` \
    --un-conc no_hg38/Saur__unmapped  `# Маска выходных ридов, которые никуда не картировались` \
...
# Просмотреть содержимое папки с картировавшимися ридами
ls hg38

# Просмотреть, сколько ридов не картировалось 
grep ' aligned 0 times' hg38/bowtie2.log

# Найти и переименовать файлы
find no_hg38/ \
    -type f \
    -exec bash -c '
        FILE="{}";
...
# Просмотреть содержимое папки с некартировавшимися ридами
ls -d no_hg38/*
```

```text
no_hg38/Saur__unmapped.1.fastq  no_hg38/Saur__unmapped.2.fastq
```
