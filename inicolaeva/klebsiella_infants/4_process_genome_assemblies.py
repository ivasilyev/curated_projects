...
for assembly_file in assembly_files:
    ...
    #
    # shutil.copy2(assembly_file, os.path.join(assemblies_target_dir, "{}_{}.fna".format(sample_name, assembly_type)))
    #
    ...
    assemblies_dict[sample_name]["{}_assembly_contigs_number".format(assembly_type)] = len(seq_records)
    assemblies_dict[sample_name]["{}_assembly_bp".format(assembly_type)] = sum([len(i) for i in seq_records])
...
numeric_col_names = [i for i in combined_statistics_df.columns if any(i.endswith(j) for j in ("bp", "number"))]
...
combined_statistics_df["genome_assembled_reads_percentage"] = combined_statistics_df["genome_assembly_bp"] * 100 / combined_statistics_df["sample_reads_bp"]
